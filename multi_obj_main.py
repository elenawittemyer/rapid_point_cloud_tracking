...
from multi_obj_baseline import baseline_main
...
def main(frames, num_clouds, density, verbose=True, get_accuracy=False):
    ...
    measured_clouds, pos_data, vel_data = get_point_clouds(0, density, num_clouds)
    ...
    i = 1
    while i<frames:
        measured_clouds, pos_data, vel_data = get_point_clouds(i, density, num_clouds, pos_data, vel_data)
        ...
        if get_accuracy == True:
            variation.append(calc_variation(pos_data, transforms))
            accuracy.append(calc_sdf_reduc(num_clouds, ordered_shapes, segmented_clouds, sdfs))
        
        i += 1
    
    if get_accuracy == True:
        print('avg variation: ' + str(np.average(np.array(variation))))
        '''
        with open("plotting/accuracy.txt", "a") as myfile:
            myfile.write(str(num_clouds) + ' ')
            myfile.write('[')
            for i in range(len(accuracy)-1):
                myfile.write(str(accuracy[i]) + ",")
            myfile.write(str(accuracy[-1]) + ']\n')
        '''
...
def calc_sdf_reduc(num_clouds, ordered_shapes, segmented_clouds, sdfs):
    init_sum = 0
    fin_sum = 0
    for j in range(num_clouds):
        if ordered_shapes[j] == 'circle':
            init_sdf = calc_cost_c(np.array([0, 0]), segmented_clouds[j])
        
        elif ordered_shapes[j] == 'square':
            init_sdf = calc_cost_sq(np.array([0, 0]), segmented_clouds[j])

        elif ordered_shapes[j] == 'triangle':
            init_sdf = calc_cost_t(np.array([0, 0]), segmented_clouds[j])
        
        elif ordered_shapes[j] == 'rhombus':
            init_sdf = calc_cost_r(np.array([0, 0]), segmented_clouds[j])

        elif ordered_shapes[j] == 'hexagon':
            init_sdf = calc_cost_h(np.array([0 ,0]), segmented_clouds[j])

        init_sum += init_sdf
        fin_sum += sdfs[j]
    
    reduc = 1 - fin_sum/init_sum
    #print('sdf reduction: ' + str(reduc))
    return reduc

def calc_variation(start_pos, transforms):
    var_sum = 0
    for i in range(len(start_pos)):
        var_sum += np.abs((transforms[i]-start_pos[i])/start_pos[i])
    var_avg = np.average(var_sum / len(start_pos))
    if math.isinf(var_avg):
        var_avg = 0
    #print('percent variation: ' + str(var_avg))
    return var_avg

################################
## test data ###################
################################
def get_point_clouds(iter, density, num_clouds, start_pos = None, vel = None):
    if start_pos is None:
        start_pos_x = numpy.random.choice(np.arange(-50, 50), num_clouds, replace=False)
        start_pos_y = numpy.random.choice(np.arange(-50, 50), num_clouds, replace=False)
        start_pos = np.hstack((np.array([start_pos_x]).T, np.array([start_pos_y]).T))

    if iter%10 == 0:
        vel = numpy.random.randint(-5, 5, num_clouds*2)
        vel = np.reshape(vel, (num_clouds, 2))
    
    start_pos = start_pos + vel
    point_cloud_c = get_circle(start_pos[0], 1, density)
    point_cloud_sq = get_rect(start_pos[1], 1, 1, density)
    point_cloud_t = get_triangle(start_pos[2], 2, 2, density)
    point_cloud_r = get_rhomb(start_pos[3], 1, 1, density)
    point_cloud_h = get_hex(start_pos[4], 1, density)
...
main(frames, num_clouds, density, verbose = False, get_accuracy=True)
solve_time = (time.time()-start_time)/20
print(solve_time)
