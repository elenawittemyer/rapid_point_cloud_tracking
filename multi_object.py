...
cloud_shapes = numpy.floor(numpy.random.uniform(0, num_shapes, num_clouds))
start_poses = (numpy.random.uniform(-10, 10, (num_clouds, 2))).reshape(5,2)
velocities = numpy.random.uniform(-.5, .5, num_clouds)

def get_point_clouds(iter):
    point_clouds = []
    for i in range(len(cloud_shapes)):
        if cloud_shapes[i]==0:
            point_clouds.append(get_circle(start_poses[i] + np.array([iter*velocities[i], iter*velocities[i]]), 1, 1000))
        else:
            point_clouds.append(get_rect(start_poses[i] + np.array([iter*velocities[i], iter*velocities[i]]), 1, 1, 1000))
    return point_clouds
